...
def replace_media(strr):
    return strt.replace("media/", "media/grades/")
...
def list_report_downloads(request, course_id):
    ...
    response_payload = {
        'downloads': [
            dict(name=name, url=url.replace("media/", "media/grades/"), link=HTML('<a href="{}">{}</a>').format(HTML(url), Text(name)))
